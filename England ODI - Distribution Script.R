...
newbattingData <- na.omit(batting)
...
# Keep updating frame with new values

England_ODI_two <- rbind(England_ODI, newbattingData)
View(England_ODI_two)
...
# Back up mutate for peaks
battingData <- na.omit(batting)
summary <- battingData %>% 
  group_by(id, age) %>% 
  mutate(av = sum(Runs)/n()) %>% 
  group_by(id) %>% 
